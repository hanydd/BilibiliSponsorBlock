 ...
 *
 * Bilibili uses SSR + Vue 2.  All DOM elements arrive pre-rendered, but Vue
 * does not attach event handlers or reactive behaviour until its mount phase
 * completes (~2 s after DOMContentLoaded based on profiling).
 *
 * We poll for the `__vue__` (Vue 2) or `__vue_app__` (Vue 3) property on
...
function detectVueMountAndNotify(): void {
    ...
    const check = () => {
        ...
        if (vue2 || vue3) {
            ...
            console.log(`${TAG} Vue mounted (vue2=${vue2}, vue3=${vue3}) at +${elapsed}ms after ${checkCount} checks`);
            window.postMessage({ source: sourceId, type: "pageReady" }, "/");
            return;
        }

        if (checkCount >= 200) {
            // ~10 s timeout (200 × 50 ms) — give up and notify anyway
            const elapsed = Math.round(performance.now() - t0);
