...
| `BSB_E2E_EXECUTABLE_PATH` | 指定真实浏览器可执行文件；测试旧版 Chromium 时同时设置 `BSB_E2E_HEADED=1` |
| `BSB_E2E_EXTENSION_PATH` | 指定已构建的 Chromium 扩展目录，默认 `dist/`，便于保留不同浏览器的构建产物 |

`BSB_E2E_DIRECT` 和 `BSB_E2E_PROXY_SERVER` 不能同时设置。

### Chromium 最低版本

Chromium 构建声明的最低版本为 **102**。2026-09-22 在 Windows 11 上使用独立用户目录实测：

| 浏览器版本 | 结果 |
| --- | --- |
| Chromium 101.0.4951.15 | 不支持动态注册 MAIN 脚本，无法识别 CID |
| Chromium 102.0.5005.40 | 移除动态页选择器的 `:has()` 依赖后，45 项离线页面测试全部通过；重启后 MAIN 注册仍生效且无重复注入 |
| Chromium 104.0.5112.48 | 修改选择器前，动态页标签因不支持 `:has()` 失败，帮助定位了兼容性边界 |
| Chromium 105.0.5195.19 | 原有 44 项离线页面测试通过，并验证重启后 MAIN 注册持久化 |
| Chromium 148.0.7778.96 | 修改选择器后，8 项缩略图回归测试通过，包括动态列表追加卡片和整体替换 |
| Edge 109.0.1518.140、153.0.4234.32 | 在 README 的真实示例视频上正确识别 19 条线上片段并实际自动跳过 |

动态页直接使用 `.bili-dyn-list` 定位列表，不再需要 Chromium 105 才支持的 `:has()`。因此完整支持下限降至动态 MAIN 注册 API 所需的 102。101 的边界测试在加入 manifest 最低版本限制前执行。Chromium 的功能测试使用本地页面和 WebM；真实 B 站视频播放另用 Edge 验证。未验证 Windows 7；102 的最低版本结论仅适用于 Chromium。

2026-09-22 补充跨浏览器回归：

- Chromium 102.0.5005.40 和官方最新稳定版 Chrome for Testing 153.0.8010.52，各自通过全部 45 项离线页面测试。
- Firefox 正式版 156.0 使用 Firefox 构建及 WebDriver 临时安装，通过 13 组关键功能检查：页面识别及刷新、自动跳过和撤销/重做、手动跳过、禁用跳过、高光、录制及编辑器、取消录制、快捷键、弹窗开关及白名单、导入、分 P/SPA 筛选、动态标签追加及列表替换、设置持久化。
- Chrome 的两项真实 B 站页面测试通过，覆盖播放器控件、弹窗和提交编辑器。Chrome 和 Firefox 在 README 示例视频上均获取到 19 条线上片段，并实际跳过 `[18.913, 32.876]`。

Firefox 验证使用独立配置及本地 HTTPS 页面、WebM 和接口响应，复用现有页面测试的数据；没有使用 Playwright 的定制 Firefox 代替正式版。测试配置关闭原生自动画中画及悬浮开关，避免切换扩展页读取存储和自动点击时触发画中画，并在独立场景间清除片段缓存。真实页面检查使用正常网络，不向线上提交片段。两种测试框架覆盖范围不同，不把 Firefox 的 13 组检查等同于完整运行 Chromium 的 45 项用例。
